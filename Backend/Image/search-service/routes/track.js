...
// 🔹 Secrets에서 값 가져오기
const REDIS_URL = readSecret('redis_url') || 'redis://itda-redis-svc.itda-redis-ns.svc.cluster.local:6379';
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!track) {
      ...
      await track.save();
      console.log("✅ 트랙을 DynamoDB에 저장했습니다:", track_id);
    } else {
      // 트랙 업데이트
      ...
      console.log("🔄 DynamoDB에서 트랙을 업데이트했습니다:", track_id);
    }
  ...
  try {
    ...
    try {
      ...
      if (cachedVideoId) {
        ...
        console.log("ℹ️ Redis에 캐시된 YouTube ID가 없습니다. DynamoDB 확인 중...");
      }
    } catch (redisErr) {
    ...
    if (track && track.streaming_id) {
      console.log("✅ DynamoDB에서 YouTube ID를 찾았습니다:", track.streaming_id);
      
      // DynamoDB에서 찾은 데이터를 Redis에 캐싱
      try {
        await redis.set(redisKey, track.streaming_id, 'EX', REDIS_CACHE_TTL);
        console.log("✅ DynamoDB의 YouTube ID를 Redis에 캐싱했습니다.");
      } catch (redisErr) {
        console.error("⚠️ Redis 캐싱 중 오류 발생:", redisErr);
      }
...
router.get('/artist/:artistId', async (req, res) => {
  ...
  try {
    console.log("🔍 아티스트 ID로 트랙 검색 중:", artistId);
    
    // DynamoDB에서 아티스트 ID로 트랙 검색
    const tracks = await Track.findByArtist(artistId);
    
    if (tracks && tracks.length > 0) {
      console.log(`✅ DynamoDB에서 ${tracks.length}개의 트랙을 찾았습니다.`);
      return res.json({ tracks });
    } else {
      console.log("❌ 해당 아티스트의 트랙을 찾을 수 없습니다.");
